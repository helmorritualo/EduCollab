...
export const createProduct = async (name: string, description: string, type: string = 'SERVICE') => {
  try {
...
export const createSubscription = async (planId: string, userEmail: string) => {
  try {
    const accessToken = await getAccessToken();
