...
export const uploadFile = async (fileUpload: FileUpload): Promise<number> => {
  try {
    const sql = `
      INSERT INTO files
...
export const getFilesByGroupId = async (
  groupId: number
...
export const getAllFiles = async (): Promise<FileUploadWithDetails[]> => {
  try {
    const sql = `
      SELECT
        f.*,
...
export const getFileById = async (
  ...
  try {
    ...
    const files = rows as FileUpload[];
    return files[0] || null;
  } catch (error) {
